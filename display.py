...
def table():
    ...
    if(ext(session['compose']) == 'LDA' or ext(session['compose']) == 'LR' or ext(session['compose']) == 'AdaBoost' or ext(session['compose']) == 'GDBT' or ext(session['compose']) == 'XGB'):
        ...
        return {'code':0, 'msg':'', 'count':0, 'data':
            [{'head':'accuracy', 's0':round(df.loc['accuracy', 's0'], 3), 's1':round(df.loc['accuracy', 's1'], 3), 's2':round(df.loc['accuracy', 's2'], 3), 's3':round(df.loc['accuracy', 's3'], 3), 's4':round(df.loc['accuracy', 's4'], 3), 's5':round(df.loc['accuracy', 's5'], 3), 's6':round(df.loc['accuracy', 's6'], 3), 's7':round(df.loc['accuracy', 's7'], 3), 's8':round(df.loc['accuracy', 's8'], 3), 'Average':round(df.loc['accuracy', 'Average'], 3), 'Standard deviation':round(df.loc['accuracy', 'Standard deviation'], 3)},
